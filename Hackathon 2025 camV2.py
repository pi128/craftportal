...
grass_img = pygame.transform.scale(
    pygame.image.load("craftportal-main\craftportal-main\Sprites\Tiles\Grass\grass-1.png").convert_alpha(),
    (tile_size, tile_size),
)
path_img = pygame.transform.scale(
    pygame.image.load("craftportal-main\craftportal-main\Sprites\Tiles\Dirt Path\dirtpath-1.png").convert_alpha(),
    (tile_size, tile_size),
)

# Character animations
def load_animation_list(file_list):
    return [pygame.image.load(f"craftportal-main\craftportal-main\Sprites\Characters\{file}").convert_alpha() for file in file_list]
...
def create_main_room():
    ...
    lay_path(layout, 10, 0, "vertical", 12)
    return layout
...
caveentrance_image = pygame.image.load("craftportal-main\craftportal-main\Sprites\Tiles\Other/brick-wall.png").convert_alpha()
add_object(objects, 10, 0, caveentrance_image)

# Add a tree
tree_image = pygame.image.load("craftportal-main\craftportal-main\Sprites\Tiles\Grass\grass-1.png").convert_alpha()
...
# Tree in main
tree_image = pygame.image.load("craftportal-main\craftportal-main\Sprites/Objects/Trees and Shrubs/tree-1.png").convert_alpha()
add_object(maps["main"]["objects"], 4, 2, tree_image)

# Barrel in lab
barrel_image = pygame.image.load("craftportal-main\craftportal-main\Sprites/Objects/Other/barrel-medium.png").convert_alpha()
...
while running:
    ...
    if (
        ...
        caveentrance_rect = caveentrance_image.get_rect()
        if caveentrance_rect.left<=player_rect.top/10<=caveentrance_rect.right:
            if map_switch_timer >= map_switch_delay:
                current_map = "lab" if current_map == "main" else "main"
                tile_layout = maps[current_map]["layout"]
                objects = maps[current_map]["objects"]
                background = maps[current_map]["background"]
                ...
                player_pos.x,player_pos.y = player_pos.x,screen_height-50
