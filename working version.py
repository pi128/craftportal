...
pygame.init()
# Detect web/pygbag runtime and avoid initializing audio, which can hang under WASM
...
while first:
    for e in pygame.event.get():
        if e.type==QUIT:
            # Avoid hard-exit on web; just break to stop the menu
            first = False
            running = False
            break
        if e.type==MOUSEBUTTONDOWN:
            if start_rect.collidepoint(e.pos):
                ...
                # Post a QUIT instead of sys.exit for pygbag compatibility
    ...
    for e in pygame.event.get():
        if e.type==QUIT:
            ...
            if pause_x_btn.get_rect(topleft=(0,0)).collidepoint(e.pos):
                ...
                # return to menu by breaking the loop rather than exiting the interpreter
                first = True
                running = False
