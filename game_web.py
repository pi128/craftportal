...
def safe_load(path, scale=None):
    try:
        ...
        size = scale if scale else (TILE, TILE)
...
async def main():
    global first, running, pause, mining, mining_target, mining_timer, mining_idx
    global current_map, tile_layout, objects, tile_img, visibility, dug
    global player_pos, facing, frame_idx, frame_timer
    global craft_msg, craft_msg_time, portal_msg, portal_msg_time, has_portal_gun
    global ore_counts, player_tool, last_mine_time
    ...
    while running:
        ...
        if mining:
            ...
            if mining_timer >= mining_duration and mining_target:
                ...
                dug[my][mx] = True
                persistent_overlays.pop((mx, my), None)

                mining = False
                mining_target = None

                # auto-craft portal-gun
        ...
        # Animate the player
        moved = (dx != 0 or dy != 0) if 'dx' in locals() else False
        if moved:
            frame_timer += dt
        ...
        # Yield control to browser for smooth performance
        if IS_WEB:
            await asyncio.sleep(0)
...
if __name__ == "__main__":
    if IS_WEB:
        ...
        # For non-web platforms, run synchronously
        import threading
        def run_sync():
